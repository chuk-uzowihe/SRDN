...
## Layout
```
srdn/         the SRDN cell + LM (model.py), MoE channel mixer, norm. First-party.
baselines/    Transformer, Mamba-3, canonical M2RNN, NVIDIA GDN-2 (external ones load from refs/).
tasks/        frjt, enwik8, graph_rl.
experiments/  frjt_compare.py, train_lm.py (enwik8), train_graph_rl.py.
tests/        parity (clean==original), chunk-equivalence (chunked==full), step-vs-parallel.
